...
import { login, AuthActionState, verifyRegistrationOtp, resendOtp } from "@/lib/actions/auth";
import { 
  Field, 
  FieldContent, 
  FieldLabel, 
  ...
  if (isVerified) {
    return (
      ...
      >
        ...
        <p className="text-muted-foreground mb-8">Your email has been successfully verified. You can now sign in to your account.</p>
        <button 
          onClick={() => window.location.reload()}
          className="btn-minimal w-full inline-block pt-3"
        >
          Sign In Now
        </button>
      </motion.div>
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground">
            Your email is not verified. We&apos;ve sent a 6-digit code to <span className="font-bold text-foreground">{state.email}</span>
          </p>
        </div>
